<script lang="ts">
  ...
  async function runAll() {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      if (last.error < toleranceInput && Math.abs(last.fx1) < toleranceInput) break;
    }
    running = false;
  }
...
<div class="space-y-6">
  <Card>
    <h2 class="text-xl font-semibold text-accent mb-4">Secant Method</h2>
    ...
    <div class="space-y-4">
      ...
      <div>
        ...
        <div class="grid grid-cols-3 gap-4 mb-4">
          <div>
            <span class="text-sm text-muted mb-1 block">Initial x&#8320;</span>
            <Input
              type="number"
        ...
        <div class="flex gap-2 mb-4">
          <Button onclick={step} disabled={running || (iterations.length > 0 && iterations[iterations.length - 1].error < toleranceInput && Math.abs(iterations[iterations.length - 1].fx1) < toleranceInput)}>
            Step
          </Button>
        ...
        {#if iterations.length > 0}
          ...
          {#if iterations.length >= 20 || (iterations[iterations.length - 1].error < toleranceInput && Math.abs(iterations[iterations.length - 1].fx1) >= toleranceInput)}
            <div class="mt-3 p-3 bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 text-sm">
              <strong>Warning:</strong> Step size converged but |f(x)| = {Math.abs(iterations[iterations.length - 1].fx1).toFixed(6)} is not near zero. The method may have stagnated at a non-root. Try different starting points.
            </div>
          {/if}

          {#if iterations[iterations.length - 1].error < toleranceInput && Math.abs(iterations[iterations.length - 1].fx1) < toleranceInput}
            <div class="mt-3 p-3 bg-green-500/10 border border-green-500/30 text-green-400 text-sm">
              Converged to root x ≈ {iterations[iterations.length - 1].x1.toFixed(6)} in {iterations.length} iterations.
            </div>
          {/if}
